...
**状态**：✅ **已完成并部署到生产环境**
**影响范围**：首页视图默认值、排名表组件、新增只读 API 接口
**严重级别**：🟢 **低（纯功能优化，不影响数据库和现有功能）**

---

### 📌 **版本概述**

本版本优化首页厂家清单的展示体验，将默认视图从卡片切换为排名表，并为排名表新增自然年筛选能力。用户可按年份查看该年发布视频的厂家播放量和视频数排名，更直观地判断各厂家当年的内容运营竞争力。所有改动为只读扩展，不修改数据库 schema，不影响现有 API。
...
#### 1. **首页默认视图：卡片 → 排名表**
- `page.tsx` 中 `useState<ViewType>` 默认值由 `"cards"` 改为 `"ranking"`
- 用户打开首页直接看到排名表，操作效率更高

#### 2. **排名表：自然年筛选**
- 新增年份按钮组（全部 / 各可用年份），点击切换后从 `/api/ranking?year=YYYY` 拉取数据
- 数据语义：该年发布的视频，取各视频**最新一次**采集的播放量之和（非跨日期累加），避免重复计数
- 切换年份时显示加载动画；选"全部"时直接复用父组件传入的全量数据，不发额外请求
- Header 区域展示当前筛选年份标签（如"2025 年发布视频"）

#### 3. **排名表：精简排序选项**
- 排序下拉仅保留"总播放量"和"视频数"
- 移除"粉丝数"和"品牌名称"排序（粉丝数由历史积累主导，不反映当年运营能力）
- 粉丝数列仍在表格中展示，仅移除了排序入口

#### 4. **新增只读 API 接口 `/api/ranking`**

| 请求 | 返回 |
|------|------|
| `GET /api/ranking` | 数据库中有视频记录的年份列表 |
| `GET /api/ranking?year=2025` | 该年发布视频的厂家聚合数据（BrandWithStats[]） |

#### 5. **新增只读 DB 函数**

| 函数 | 说明 |
|------|------|
| `getBrandsWithStatsByYear(year)` | 按 `videos.pub_date` 过滤年份，取各视频最新播放量聚合 |
| `getAvailableYears()` | 返回 `videos` 表中有 `pub_date` 记录的年份列表 |
...
- 未修改任何现有数据库表结构或数据
- 未修改任何现有 API 接口（`/api/overview`、`/api/brands` 等不变）
- 年份筛选数据源独立，"全部"状态完全回退到原有数据 prop
- `BrandRankingTable` 的 `data` prop 接口不变，调用方 `page.tsx` 无需改动传参

---

### 📝 **验证方式**

1. 打开首页，确认默认展示排名表（非卡片视图）
2. 点击年份按钮（如 2025），确认排名数据切换为该年发布视频统计
3. 点击"全部"，确认恢复全量数据
4. 确认排序下拉只有"总播放量"和"视频数"两个选项
5. 确认其他页面（品牌详情、对比页、厂家管理）功能正常
...
- 或将 `page.tsx` 第34行改回 `useState<ViewType>("cards")`

---

## 🔧 v2.33.0-login-time-fix
...
**修复时间**：2026-05-10 21:50 左右

用户反馈生产环境存在两个业务问题：

- 首页「添加厂家」能完成 MID 识别，但点击确认后没有成功添加；
- 多个品牌详情页（例如 `/brand/1`）月度数据排序错误，发布趋势图展示的月份和数据不符合最近 12 个月预期。

**根因定位**：

- ✅ 首页和全局 Layout 的 `AddBrandModal` 只绑定了刷新 overview 的函数，没有真正调用 `POST /api/brands`；
- ✅ 品牌管理页虽然会调用添加接口，但错误提示过于笼统，重复 MID 等后端错误无法透传到弹窗；
- ✅ 后端 `getBrandMonthlyStats()` 从品牌最早月份开始补齐月份，并在 24 个月后停止，导致历史较长的品牌只返回早期月份；
- ✅ 前端品牌详情页又基于这批错误月份生成「近12个月」趋势图，最终造成表格和图表都偏离当前时间窗口。

**修复内容**：

- ✅ 首页 `/` 添加厂家入口改为真实提交 `POST /api/brands`，成功后刷新 overview；
- ✅ 全局 Dashboard Layout 添加厂家入口改为真实提交 `POST /api/brands`，不再只刷新数据；
- ✅ 厂家管理页添加流程透传后端错误信息；
- ✅ `/api/brands` 新增 MID 去重校验，重复添加时返回 `409` 和明确提示 `厂家已存在：xxx`；
- ✅ `getBrandMonthlyStats()` 改为固定生成最近 12 个月数据，缺失月份补 0；
- ✅ 品牌详情页趋势图使用后端返回的最近 12 个月升序数据；
- ✅ 品牌详情页月度表格使用同一份数据倒序展示，确保图表和表格口径一致。

**部署与验证**：

- ✅ 本次修改文件 ESLint 通过；
- ✅ 本地 `npm run build` 通过；
- ✅ 服务器 `/opt/embodied-marketing` 执行 `npm run build` 通过；
- ✅ PM2 重启 `embodied-marketing` 成功并保持 `online`；
- ✅ 本地与服务器 6 个关键运行文件 SHA-256 哈希一致；
- ✅ 服务器按新算法复算 `/brand/1` 月份范围为 `2025-06` 到 `2026-05`；
- ✅ 本地提交并推送 GitHub，最新提交为 `b307224 fix(brands): repair add flow and monthly stats`。

#### 4. 每日自动采集任务复核与权限修复

**检查时间**：2026-05-10 22:00 左右

对服务器自动采集任务进行复核：

- ✅ 确认 root crontab 已配置每日凌晨自动采集任务；
- ✅ 实际执行时间为每天 `00:30`，任务为 `/opt/embodied-marketing/scripts/daily_collect.sh`；
- ✅ 确认服务器时区为 `Asia/Shanghai`，cron 服务为 `active running`；
- ✅ 确认未安装项目相关 systemd timer，当前线上自动采集以 root crontab 为准；
- ✅ 发现 `daily_collect.sh` 缺少执行权限，cron 直接调用时可能 `Permission denied`；
- ✅ 已在服务器执行 `chmod +x /opt/embodied-marketing/scripts/daily_collect.sh`；
- ✅ 已同步本地 `bilibili-monitor/scripts/daily_collect.sh` 可执行权限，避免后续部署覆盖回非可执行状态；
- ✅ 通过 `bash -n /opt/embodied-marketing/scripts/daily_collect.sh` 完成脚本语法检查。
...
**修复时间**：2026-05-10 23:20 左右

对厂家删除权限进行二次复核后确认：

- ✅ 后端 `DELETE /api/brands/[id]` 已强制 `requireAdmin(request)`；
- ✅ 厂家管理页 `/brands` 已按 `session.user.role === "admin"` 显示删除按钮；
- ✅ 服务器用户表当前仅 `Ash` 为 `admin` 且状态为 `active`；
- ⚠️ 品牌详情页 `/brand/[id]` 的“更多 -> 删除此品牌”入口此前未按管理员隐藏，普通用户点击后会被后端拒绝，但前端仍存在误导。

**修复内容**：

- ✅ 品牌详情页新增 session 读取逻辑，判断当前用户是否为 `admin`；
- ✅ 仅管理员显示“更多/删除此品牌”菜单；
- ✅ 删除点击和确认删除函数均增加 `isAdmin` 保护；
- ✅ 保持后端 `requireAdmin` 作为最终权限边界，前端只做体验一致性收口。
...
- ✅ 服务器 `/opt/embodied-marketing` 执行 `npm run build` 通过；
- ✅ PM2 重启 `embodied-marketing` 成功并保持 `online`；
- ✅ 本地与服务器 `src/app/brand/[id]/page.tsx` SHA-256 哈希一致；
- ✅ 本地提交并推送 GitHub，最新提交为 `2a718ed fix(brands): hide detail delete action for non-admins`。
...
> **来源**: [docs/版本更新记录.md](docs/版本更新记录.md)
> **整理日期**: 2026-05-09
> **用途**: 为架构师和高级开发者提供深度的技术决策参考

---

### A. v2.20.0 性能基准对比（2025-12-15）

#### 性能提升量化指标

| 指标 | v2.19.0 (旧) | v2.20.0 (新) | 提升幅度 |
|------|-------------|-------------|---------|
| **首屏加载时间** | 3.2s | 2.1s | ⬇️ **34%** |
| **首页加载（二次访问）** | 2.1s | 0.6s | ⬇️ **71%** |
| **品牌对比页渲染** | 3.2s | 1.2s | ⬇️ **62%** |
| **图表交互响应** | 800ms | 150ms | ⬇️ **81%** |
| **单次全量采集耗时** | 12分钟 | 6分钟 | ⬇️ **50%** |
| **并发用户支持** | 10人 | 50人+ | ⬆️ **400%** |
| **Lighthouse性能评分** | 65 | 92 | ⬆️ **42%** |

*测试环境*: Chrome 120 / MacBook Pro M2 / WiFi 100Mbps
...
const nextConfig: NextConfig = {
  compress: true,  // 启用Gzip/Brotli压缩
  experimental: {
    optimizePackageImports: ['lodash', 'recharts'], // Tree-shaking
  }
};
```

**2. 后端接口优化**
- 数据库查询添加复合索引：`CREATE INDEX idx_videos_brand_published ON videos(brand_id, published_at)`
- API响应启用强缓存：`Cache-Control: public, s-maxage=3600`
- 分页查询改为游标分页（Cursor Pagination），大数据量下性能提升50%

**3. 采集效率优化**
- 使用 `aiohttp` 异步HTTP库替代同步 `requests`
- 批量INSERT替代单条插入（事务提交）
- 内存占用降低约40%

---

### B. v2.0.0 架构重构深度解析（2025-11-10）
...
**视图一：趋势分析图 (Trend Chart)**
- 多维度指标时序分析（播放量/点赞数/收藏数/评论数/分享数）
- 支持折线图、面积图、柱状图切换
- 时间粒度智能聚合（按天/周/月）

**视图二：对比矩阵表 (Comparison Matrix)**
- 横向多品牌综合表现对比
- 智能排名算法：`综合得分 = 播放量×0.3 + 点赞×0.25 + 收藏×0.2 + 评论×0.15 + 分享×0.1`
- 热力图单元格可视化（数值越大颜色越深）

**视图三：分布散点图 (Distribution Scatter Plot)**
- 双维度相关性分析（如播放量 vs 点赞率）
- 气泡大小映射第三维度（视频时长）
- 缩放/平移/框选交互

---

### C. 数据迁移最佳实践（v1.x → v2.0）

#### 必须执行的迁移步骤

```bash
# Step 1: 备份（不可逆操作！）
cp bilibili_monitor.db backup_v1_$(date +%F).db

# Step 2: Schema迁移
python scripts/migrate_v1_to_v2.py --source ../bilibili-monitor-v1/bilibili_monitor.db

# Step 3: 验证数据完整性
python scripts/validate_migration.py

# Step 4: 配置文件转换
python scripts/convert_config_v1_to_v2.py
```
...
| 文档名称 | 路径 | 用途 |
|---------|------|------|
| **部署运维手册** | [docs/部署运维手册.md](docs/部署运维手册.md) | 服务器配置、安全加固、监控告警 |
| **数据采集指南** | [docs/数据采集指南.md](docs/数据采集指南.md) | 采集原理、调优、故障排查 |
| **产品需求文档** | [docs/产品需求文档.md](docs/产品需求文档.md) | 业务背景、功能规格、用户故事 |
| **技术规格文档** | [docs/技术规格文档.md](docs/技术规格文档.md) | 架构设计、数据库Schema、API定义 |
